...
const managePromotions = async(currentDate, promotionId) => {
    ...
    try {
        let promotionQueryRes = await customerRepository.getPromotionDetailsById(promotionId);
        const {benefit_percentage:benefitPercentage, promotion_type:promotionType} = promotionQueryRes.rows[0];
        ...
        response.data = {
            promotionType, benefitPercentage
        }
        return response;
    } catch (e) {
...
const enrollUser = async(planDetails) => {
    const response = { status: false };
    try {
        const {promotionApplied, planId, userId, promotionId, startedDate, depositedAmount} = planDetails;
        ...
        if (promotionApplied) {
           promotionValidityResponse = await managePromotions(startedDate, promotionId);
        ...
        const {amount_options:amount, tenure_options, benefit_type } = isPlanValidQueryRes.rows[0];
